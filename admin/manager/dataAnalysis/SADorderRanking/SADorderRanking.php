...
#列出有訂單的供應商ID
$SQLsad = "SELECT * FROM Supplier_OrderContent 
           LEFT JOIN Supplier_OrderTitle
           ON Supplier_OrderContent.SOC_SOT_OrderNum  = Supplier_OrderTitle.SOT_OrderNum
           WHERE 1 = 1
           $SQLplus
           group by SOC_SAD_ID";  
$RSsad = $Language_db -> query($SQLsad);
$DATAsad = $RSsad -> fetchall();

#根據供應商ID，加總全部訂單金額
foreach($DATAsad as $key=>$val) {
  $SQLsadPriceTotal = "select sum(SOC_Price*SOC_Amount) FROM Supplier_OrderContent 
                       LEFT JOIN Supplier_OrderTitle
                       ON Supplier_OrderContent.SOC_SOT_OrderNum  = Supplier_OrderTitle.SOT_OrderNum
                       where SOC_SAD_ID = '".$val['SOC_SAD_ID']."'
                       $SQLplus
                       ";
  $RSsadPriceTotal = $Language_db -> query($SQLsadPriceTotal);
  $DATAsadPriceTotal = $RSsadPriceTotal -> fetch();
  if($DATAsadPriceTotal) {
    $arrayClick[$val['SOC_SAD_ID']] = $DATAsadPriceTotal['sum(SOC_Price*SOC_Amount)']; //sad:priceTotal
  }
}
...
function drawChart() {
  ...
  foreach($arrayClick as $key=>$val) {
    #用sadid找出供應商
    $SADname = sadAccountSearch($Config_db,$key);
...
var options = {
  title: '供應商銷售排行 / Supplier Ranking',
  chartArea: {width: '60%'},
  hAxis: {
    title: '前20名排列',
    minValue: 0
              ...
              <div class="tableWarp">
                ...
                <table id="moredData" style="display:none;">  
                  <tr>
                    ...
                    <td class="num">供應商</td>
                    <td class="num" style="width:20px;">金額</td>
                  </tr>
                  <?php
                  $i=0;
                  foreach($arrayClick as $key=>$val) {
                  ...
                  <tr>
                    <td class="num"><?= $i+1; ?></td>
                    <td><a href='page_index.php?pageData=supplierDataManager&secondURL=editTw&id=<?= $key; ?>' target="_blank"><?= $SADname; ?></a></td>
                    <td><?= $val; ?></td>
                  </tr>
                  <?php
                  $i++;
