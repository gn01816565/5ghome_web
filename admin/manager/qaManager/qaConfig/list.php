      ...
      <section id="rightWarp">
        ...
        <div id="pageIndexWarp" class="boxWarp">
          <div id="toolsBar" class="boxWarp">
            <button class="blue" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=add'">新增問答</button>
          </div>
              ...
              <div class="tableWarp">
                <table>
                  <tr>
                    ...
                    <td class="txt titleTxt" style="width:150px;">分類</td>
                    <td class="txt titleTxt">問題</td>
                  ...
                  #搜尋出所屬資料全部的數量
                  #資料庫、資料表
                  $all_num = allTableNum($Config_db,'Admin_QAContent'); 
                  $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                  #列出紀錄資料
                  $sqlContent = "SELECT * FROM Admin_QAContent ORDER BY AQAC_ID  DESC  limit $star_num, $read_num";
                  ...
                  <tr>
                    ...
                    <td>
                      <h3>
                        <?php
                        $mainSql = "select * from Admin_QAMainClass WHERE  AQAMC_ID = '".$dataContent['AQAC_AQAMC_ID']."' ";
                        $mainRs = $Config_db->query($mainSql);
                        $mainData = $mainRs->fetch();
                        echo $mainData['AQAMC_Name'];
                        ?>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?=$dataContent['AQAC_Question'];?>
                      </h3>
                    </td>
                    <td>
                      <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataContent['AQAC_ID'];?>'">修改</button>
                    </td>
                    <td>
                      <button class="red toolsBtn" onclick='delSubmit(<?=$dataContent['AQAC_ID'];?>)'>刪除</button>
                    </td>
                  </tr>
                  <?php
