      ...
      <section id="rightWarp">
        ...
        <div id="pageIndexWarp" class="boxWarp">
          <div id="toolsBar" class="boxWarp">
            <button class="blue" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=add'">新增類別</button>
          </div>
            ...
            <div id="formTable">
              <div class="tableWarp">
                <table>
                  ...
                  #搜尋出所屬資料全部的數量
                  #資料庫、資料表
                  $all_num = allTableNum($Config_db,'Admin_QAMainClass'); 
                  $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                  #列出紀錄資料
                  $sqlContent = "SELECT * FROM Admin_QAMainClass ORDER BY AQAMC_ID  DESC  limit $star_num, $read_num";
                  ...
                  <tr>
                    ...
                    <td>
                      <h3>
                        <?=$dataContent['AQAMC_Name'];?>
                      </h3>
                    </td>
                    <td>
                      <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataContent['AQAMC_ID'];?>'">修改</button>
                    </td>
                    <td>
                      <button class="red toolsBtn" onclick='delSubmit(<?=$dataContent['AQAMC_ID'];?>)'>刪除</button>
                    </td>
                  </tr>
                  <?php
