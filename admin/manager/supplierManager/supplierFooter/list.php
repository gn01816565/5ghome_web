            ...
            <div id="formTable">
              <div class="tableWarp">
                <table>
                  ...
                  #搜尋出所屬資料全部的數量
                  #資料庫、資料表
                  $all_num = allTableNum($Config_db,'Supplier_IndexFooterManager'); 
                  $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                  #列出紀錄資料
                  $sqlContent = "SELECT * FROM Supplier_IndexFooterManager ORDER BY SIFM_ID DESC  limit $star_num, $read_num";
                  ...
                  <tr>
                    ...
                    <td>
                      <h3>
                        <a href='<?= $dataContent['SIFM_Url']; ?>' target="_blank" >
                          <img src='../images/supplier/<?=fiveSadAccountSearch($Config_db,$dataContent['SIFM_SAD_ID'])[1];?>/footer/<?=$dataContent['SIFM_Img'];?>' style='max-width:100%;'>
                        </a>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?=sadAccountSearch($Config_db,$dataContent['SIFM_SAD_ID']);?>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?php
                        #判斷管理者是否有下架
                        if($dataContent['SIFM_CheckStatus']=='N') {
                          echo "<text style='color:red'>管理者下架</text>";
                        } else {
                          #管理者沒下架，則判斷供應商是否有下架
                          if($dataContent['SIFM_CheckStatus']=='Y') {
                    ...
                    <td class="date">
                      <h3>
                        <?=$dataContent['SIFM_UpTime'];?>
                      </h3>
                    </td>
                    <td>
                       <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataContent['SIFM_ID'];?>'">修改</button>
                    </td>
                  </tr>
                  <?php
