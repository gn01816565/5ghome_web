      ...
      <section id="rightWarp">
        ...
        <div id="pageIndexWarp" class="boxWarp">
          <div id="toolsBar" class="boxWarp">
            <button class="blue" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=add'">新增第三方平台資訊</button>
          </div>
          <div id="newsWarp" class="boxWarp">
            ...
            <div class="tableWarp">
              <table>
                ...
                //$all_num = allTableNum($Language_db,'Admin_OtherPlatforms'); 
                $all_num = 2;
                $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                #列出紀錄資料
                $sqlList = "SELECT * FROM Admin_OtherPlatforms ORDER BY AOP_ID DESC ";
                $rsList = $Config_db->query($sqlList);
                ...
                <tr>
                  ...
                  <td>
                    <h3>
                      <img src="../images/otherPlatFormIcon/<?=$dataList['AOP_Icon'];?>" style='width:50px;'>
                    </h3>
                  </td>
                  <td>
                    <h3><?=$dataList['AOP_PlatformName'];?></h3>
                  </td>
                  <td>
                    <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataList['AOP_ID'];?>'">修改</button>
                  </td>
                  <td>
                    <button class="red toolsBtn" onclick='delSubmit(<?=$dataList['AOP_ID'];?>)'>刪除</button>
                  </td>
                </tr>
                <?php
