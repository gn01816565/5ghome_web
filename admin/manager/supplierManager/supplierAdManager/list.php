              ...
              <div class="tableWarp">
                <table>
                  <tr>
                    ...
                    <td class="txt titleTxt" style='width:100px;'>廣告狀態</td>
                  ...
                  #搜尋出所屬資料全部的數量
                  #資料庫、資料表
                  $all_num = allTableNum($Config_db,'Supplier_IndexIconManager'); 
                  $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                  #列出紀錄資料
                  $sqlContent = "SELECT * FROM Supplier_IndexIconManager ORDER BY SIIM_ID DESC  limit $star_num, $read_num";
                  ...
                  <tr>
                    ...
                    <td>
                      <h3>
                        <a href='<?= $dataContent['SIIM_Url'];?>' target='_blank'>
                          <img src='../images/supplier/<?=fiveSadAccountSearch($Config_db,$dataContent['SIIM_SAD_ID'])[1];?>/adImage/<?=$dataContent['SIIM_Img'];?>' style='max-width:100%;'>
                        </a>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?=sadAccountSearch($Config_db,$dataContent['SIIM_SAD_ID']);?>
                      </h3>
                    </td>
                    <td>
                      <h3>  
                        <?php
                        #判斷管理者是否有下架
                        if($dataContent['SIIM_CheckStatus']=='N') {
                          echo "<text style='color:red'>管理者下架</text>";
                        } else {
                          #管理者沒下架，則判斷供應商是否有下架
                          if($dataContent['SIIM_CheckStatus']=='Y') {
                            echo "<text style='color:blue'>上架中</text>";
                          } else {
                            echo "<text style='color:red'>供應商自行下架</text>";
                          }
                        }
                        ?>
                      </h3>
                    ...
                    <td class="date">
                      <h3>
                        <?=$dataContent['SIIM_UploadDate'];?>
                      </h3>
                    </td>
                    <td>
                       <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataContent['SIIM_ID'];?>'">修改</button>
                    </td>
                  </tr>
                  <?php
