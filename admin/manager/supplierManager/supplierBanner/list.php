              ...
              <div class="tableWarp">
                <table>
                  <tr>
                    ...
                    <td class="txt titleTxt" style='width:100px;'>商品狀態</td>
                  ...
                  #搜尋出所屬資料全部的數量
                  #資料庫、資料表
                  $all_num = allTableNum($Config_db,'Supplier_IndexBannerManager'); 
                  $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                  #列出紀錄資料
                  $sqlContent = "SELECT * FROM Supplier_IndexBannerManager ORDER BY SIBM_ID DESC  limit $star_num, $read_num";
                  ...
                  <tr>
                    ...
                    <td>
                      <h3>
                        <a href='<?= $dataContent['SIBM_Url'];?>' target='_blank'>
                          <img src='../images/supplier/<?=fiveSadAccountSearch($Config_db,$dataContent['SIBM_SAD_ID'])[1];?>/banner/<?=$dataContent['SIBM_Img'];?>' style='max-width:100%;'>
                        </a>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?=sadAccountSearch($Config_db,$dataContent['SIBM_SAD_ID']);?>
                      </h3>
                    </td>
                    <td>
                      <h3>  
                        <?php
                        #判斷管理者是否有下架
                        if($dataContent['SIBM_CheckStatus']=='N') {
                          echo "<text style='color:red'>管理者下架</text>";
                        } else {
                          #管理者沒下架，則判斷供應商是否有下架
                          if($dataContent['SIBM_CheckStatus']=='Y') {
                    ...
                    <td class="date">
                      <h3>
                        <?=$dataContent['SIBM_UpTime'];?>
                      </h3>
                    </td>
                    <td>
                       <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataContent['SIBM_ID'];?>'">修改</button>
                    </td>
                  </tr>
                  <?php
