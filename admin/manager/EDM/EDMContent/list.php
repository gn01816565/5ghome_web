      ...
      <section id="rightWarp">
        ...
        <div id="pageIndexWarp" class="boxWarp">
          <div id="toolsBar" class="boxWarp">
            <button class="blue" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=add'">新增電子報</button>
          </div>
            ...
            <div class="tableWarp">
              <table>
                <tr>
                  ...
                  <td class="txt titleTxt">電子報編號</td>
                ...
                $all_num = allTableNum($Language_db,'Admin_EdmContent'); 
                $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                #列出紀錄資料
                $sqlList = "SELECT * FROM Admin_EdmContent ORDER BY AEC_ID DESC limit $star_num, $read_num";
                ...
                <tr>
                  <td class="num"><?=$i+1;?></td>
                  <td>
                    <h3><?=$dataList['AEC_Num'];?></h3>
                  </td>
                  <td>
                    <h3>
                      <h3><?=$dataList['AEC_Title'];?></h3>   
                    </h3>
                  </td>
                  <td>
                    <button class="blue toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=read&id=<?=$dataList['AEC_ID'];?>'">查閱</button>
                  </td>
                  <td>
                    <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataList['AEC_ID'];?>'">修改</button>
                  </td>
                  <td>
                    <button class="red toolsBtn" onclick='delSubmit(<?=$dataList['AEC_ID'];?>)'>刪除</button>
                  </td>
                </tr>
                <?php
                } //for($i=0;$dataList = $rsList->fetch();$i++) {
