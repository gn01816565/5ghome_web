      ...
      <section id="rightWarp">
        ...
        <div id="pageIndexWarp" class="boxWarp">
          <div id="toolsBar" class="boxWarp">
            <button class="blue" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=add'">新增公告</button>
          </div>
            ...
            <div class="tableWarp">
              <table>
                <tr>
                  ...
                  <td class="txt titleTxt" style="width:100px;">建立日期</td>
                  <td class="txt titleTxt" style="width:100px;">建立人員</td>
                ...
                #搜尋出所屬資料全部的數量
                #資料庫、資料表
                $all_num = allTableNum($Language_db,'Admin_IndexNews'); 
                $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 

                #列出紀錄資料
                $sqlAIN = "SELECT * FROM Admin_IndexNews ORDER BY  AIN_ID  DESC  limit $star_num, $read_num";
                $rsAIN = $Language_db->query($sqlAIN);

                for($i=0;$dataAIN = $rsAIN->fetch();$i++) {
                ?>
                <tr>
                  <td class="num"><?=$i+1;?></td>
                  <td class="leftTxt" style="padding-left:10px;">
                    <h3>
                      <?=$dataAIN['AIN_Title'];?>
                    </h3>
                  </td>
                  ...
                  <td>
                    <h3>
                      <?php
                      if($dataAIN['AIN_Status']=='Y') {
                        echo "<text style='color:blue;'>已發佈</text>";
                      } else {
                        echo "<text style='color:red;'>未發佈</text>";
                      }
                      ?>
                    </h3>
                  </td>
                  <td>
                    <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataAIN['AIN_ID'];?>'">修改</button>
                  </td>
                  <td>
                    <button class="red toolsBtn" onclick='delSubmit(<?=$dataAIN['AIN_ID'];?>)'>刪除</button>
                  </td>
                </tr>
                <?php
                } //for($i=0;$dataAIN = $rsAIN->fetch();$i++) {
