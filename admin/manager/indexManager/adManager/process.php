...
if($act == 'add') { //新增
  ...
  if(!empty($_FILES['addFile']['name'])){ //如果不是空值，則進入檔案上傳
    #檔案名稱、上傳路徑、檔案標頭 
    $file_name = uploadFile('addFile','../../../../images/adImage/','AD'); //回傳更改過後的上傳檔名
  ...
  $sqladd = "INSERT INTO Admin_IndexIconManager (AIIM_Img, AIIM_Url, AIIM_UploadDate, AIIM_Sort, AIIM_AM_ID) VALUES ('".$file_name."', '".$imgURL."', '".$date."', '1', '".$amID."');";
...
if($act == 'edit') {
  ...
  if(!empty($_FILES['addFile']['name'])){ //如果不是空值，則進入檔案上傳
    #先刪除既有圖片
    $fileURL = "../../../../images/adImage/".$_POST['fileName'];
  ...
  $sqledit = "UPDATE Admin_IndexIconManager SET AIIM_Url = '".$imgURL."', AIIM_UploadDate = '".$date."' $sqlPlus WHERE AIIM_ID = '".$bid."'";
...
if($act == 'del') { //刪除
  ...
  #抓取資料庫中的檔案名稱
  $sql = "SELECT AIIM_ID,AIIM_Img FROM Admin_IndexIconManager where AIIM_ID = '".$id."'";
  $rs = $Language_db->query($sql);
  $data = $rs->fetch();
  $fileURL = "../../../../images/adImage/".$data['AIIM_Img'];
  
  #資料庫語法
  $count = $Language_db->exec("DELETE FROM Admin_IndexIconManager WHERE AIIM_ID = '".$id."'");
