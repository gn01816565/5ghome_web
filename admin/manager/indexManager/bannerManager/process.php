...
if($act == 'add') { //新增
  ...
  if(!empty($_FILES['addFile']['name'])){ //如果不是空值，則進入檔案上傳
    #檔案名稱、上傳路徑、檔案標頭 
    $file_name = uploadFile('addFile','../../../../images/bannerImage/','Banner'); //回傳更改過後的上傳檔名
  ...
  $sqladd = "INSERT INTO Admin_IndexBannerManager (AIBM_Img, AIBM_Url, AIBM_UploadDate, AIBM_Sort, AIBM_AM_ID) VALUES ('".$file_name."', '".$imgURL."', '".$date."', '1', '".$amID."');";
...
if($act == 'edit') {
  ...
  if(!empty($_FILES['addFile']['name'])){ //如果不是空值，則進入檔案上傳
    #先刪除既有圖片
    $fileURL = "../../../../images/bannerImage/".$_POST['fileName'];
  ...
  $sqledit = "UPDATE Admin_IndexBannerManager SET AIBM_Url = '".$imgURL."', AIBM_UploadDate = '".$date."' $sqlPlus WHERE AIBM_ID = '".$bid."'";
...
if($act == 'del') { //刪除
  ...
  #抓取資料庫中的檔案名稱
  $sql = "SELECT AIBM_ID,AIBM_Img FROM Admin_IndexBannerManager where AIBM_ID = '".$id."'";
  $rs = $Language_db->query($sql);
  $data = $rs->fetch();
  $fileURL = "../../../../images/bannerImage/".$data['AIBM_Img'];
  
  #資料庫語法
  $count = $Language_db->exec("DELETE FROM Admin_IndexBannerManager WHERE AIBM_ID = '".$id."'");
