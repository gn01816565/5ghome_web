...
function switchStatus($status) {
  ...
  switch($status) {
    case 'noShipping':
      $returnStr = "<span class='red' style='width:100px;padding:5px 13px 5px 13px;'>未出貨</span>";
      break;
    case 'shipped':
      $returnStr = "<span style='padding:5px 13px 5px 13px;border-style:solid;border-width:1px;font-weight:bold;'>已出貨</span>";
      break;
    case 'refund':
      $returnStr = "<span class='yellow' style='padding:5px 5px 5px 5px;'>退貨/退款</span>";
      break;
    case 'replacement':
      $returnStr = "<span class='green' style='padding:5px 21px 5px 21px;'>換貨</span>";
      break;
    case 'complaints':
      $returnStr = "<span class='blue' style='padding:5px 21px 5px 21px;'>客訴</span>";
      break;
    case 'invaild':
      $returnStr = "<span class='red2' style='padding:5px 21px 5px 21px;'>作廢</span>";
      break;
    default:
      $returnStr = "N";
  }
        ...
        <div id="pageIndexWarp" class="boxWarp">
          ...
          <div id="toolsBar" class="boxWarp">
            <form method="post" action="#">
              <select name="supplier" style='float:left;height: 34px;background-color: #fff;'>
              ...
              <select name="status" style='float:left;height: 34px;background-color: #fff;margin-left:5px;'>
                ...
                <option value="noShipping" <?php if($status=='noShipping') { echo "selected"; } ?>>未出貨</option>
                <option value="shipped" <?php if($status=='shipped') { echo "selected"; } ?>>已出貨</option>
                <option value="refund" <?php if($status=='refund') { echo "selected"; } ?>>退貨/退款</option>
                <option value="replacement" <?php if($status=='replacement') { echo "selected"; } ?>>換貨</option>
                <option value="complaints" <?php if($status=='complaints') { echo "selected"; } ?>>客訴</option>
                <option value="invaild" <?php if($status=='invaild') { echo "selected"; } ?>>作廢</option>
              </select>
                ...
                <table>
                  ...
                  <tr>
                    ...
                    <td>
                      <h3>
                        <?php
                         ...
                         echo $dataContent['SOT_OrderNumCode'];
                         ?>
                      </h3>
                    </td>
                    <td>
                      <h3 style='word-break: break-all;'>
                        <?php
                        $SQLorderStatus = "SELECT * FROM Supplier_OrderStatus WHERE SOS_SOT_OrderNum LIKE '".$dataContent['SOT_OrderNum']."' ";
                        $RSorderStatus = $Language_db -> query($SQLorderStatus);
                        $DATAorderStatus = $RSorderStatus -> fetch();
                        echo sadAccountSearch($Config_db,$DATAorderStatus['SOS_SAD_ID']);
                        ?>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?= $dataContent['SOT_Name']; ?>
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?php
                          #算訂單總金額
                          $sqlProPrice = "select SUM(SOC_Price*SOC_Amount) from Supplier_OrderContent where SOC_SOT_OrderNum = '".$dataContent['SOT_OrderNum']."'";
                          $rsProPrice = $Language_db->query($sqlProPrice);
                          $dataproPrice = $rsProPrice->fetch();
                          echo $dataproPrice['SUM(SOC_Price*SOC_Amount)'];
                        ?>
                      </h3>
                    </td>
                    <td>
                        <?php
                          echo switchStatus($dataContent['SOT_OrderStatus']);
                        ?>
                    ...
                    <td>
                      <h3>
                        <?php
                          echo switchPay($dataContent['SOT_ProductStatus']);
                        ?> 
                      </h3>
                    </td>
                    <td>
                      <h3>
                        <?php
                          echo switchPay($dataContent['SOT_Freight']);
                        ?>  
                      </h3>
                    </td>
                    <!--
                    ...
                    -->
                    <td>
                      <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataContent['SOT_OrderNum'];?>'">修改</button>
                    </td>
                    <td>
