...
if($act == 'search') {
  ...
  if($startTime) { //開始時間
    $SQLplus .= " AND ";
    $SQLplus .= " RCP_AddDate >= '".$startTime."'";
  }

  if($endTime) { //結束時間
    $SQLplus .= " AND ";
    $SQLplus .= " RCP_AddDate <= '".$endTime."'";
  }
} //if($act == 'search') {

#抓出頁面點擊數量，先把重覆的網址濾掉
$SQLAllPageClick = "SELECT * FROM  Record_ClickPage where 1 = 1 $SQLplus ORDER BY RCP_ID DESC ";
$RSAllPageClick = $Config_db->query($SQLAllPageClick);
$dataAllPageClick = $RSAllPageClick -> fetchAll();

#將全部的網址數量做加總，先找出相同的網址做點擊數量的加總
//for($i = 0; $i <= count($dataAllPageClick); $i++ ) {
foreach($dataAllPageClick as $key => $val) {
  $SQLAmountPageClick = "SELECT SUM(RCP_Amount) FROM  Record_ClickPage WHERE RCP_PageURL like '".$val['RCP_PageURL']."' $SQLplus";
  $RSAmountPageClick = $Config_db->query($SQLAmountPageClick);
  $dataAmountPageClick = $RSAmountPageClick->fetch();
  //echo $dataAmountPageClick['SUM(RCP_Amount)'];
  $arrayClick[$val['RCP_PageURL']] = $dataAmountPageClick['SUM(RCP_Amount)'];
}
...
var options = {
  title: '頁面點擊排行 / Page Click Ranking',
