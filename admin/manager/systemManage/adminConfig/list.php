      ...
      <section id="rightWarp">
        ...
        <div id="pageIndexWarp" class="boxWarp">
          <div id="toolsBar" class="boxWarp">
            <button class="blue" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=add'">新增管理帳號</button>
          </div>
              ...
              <div class="tableWarp">
                <table>
                  <tr>
                    ...
                    <td class="txt titleTxt" style="width:100px;">帳號</td>
                    <td class="txt titleTxt" >權限</td>
                    <td class="txt titleTxt" style="width:100px;">狀態</td>
                  ...
                  #搜尋出所屬資料全部的數量
                  #資料庫、資料表
                  $all_num = allTableNum($Language_db,'Admin_Manager'); 
                  $pageAll_num = ceil($all_num / $read_num); //頁碼數計算，全部數量/讀取數量 
                  #列出紀錄資料
                  $sqlAM = "SELECT * FROM Admin_Manager ORDER BY  AM_ID  DESC  limit $star_num, $read_num";
                  ...
                  <tr>
                    <td class="num"><?=$i+1;?></td>
                    <td>
                      <h3><?=$dataAM['AM_Account'];?></h3>
                    </td>
                    <td class="leftTxt">
                      <h3>
                        <?php
                        $comp = explode(",", $dataAM['AM_Competence']); //將字串陣列化
                        for($j=0;$j<count($comp);$j++) {
                          $compSql="select AMC_EnName,AMC_TwName from Admin_ManagerCompetence where AMC_EnName = '".$comp[$j]."' ";
                          $compRs = $Config_db->query($compSql);
                          $compData = $compRs->fetch();
                          echo  $compData['AMC_TwName'];
                    ...
                    <td>
                      <h3>
                        <?php
                        if($dataAM['AM_Status']=='Y') {
                          echo "<text style='color:blue;'>正常</text>";
                        } else {
                          echo "<text style='color:red;'>停權</text>";
                        }
                        ?>
                      </h3>
                    </td>
                    <td>
                      <button class="yellow toolsBtn" onclick="location.href='page_index.php?pageData=<?=$_GET['pageData'];?>&secondURL=edit&id=<?=$dataAM['AM_ID'];?>'">修改</button>
                    </td>
                    <td>
                      <button class="red toolsBtn" onclick='delSubmit(<?=$dataAM['AM_ID'];?>)'>刪除</button>
                    </td>
                  </tr>
                  <?php
